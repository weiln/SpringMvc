...
import com.elf.soap.soapmap.engine.exchange.DataExchange;
import com.elf.soap.soapmap.engine.impl.SoapMapClientImpl;
import com.elf.soap.soapmap.engine.impl.SoapMapExecutorDelegate;
import com.elf.soap.soapmap.engine.mapping.result.loader.ResultLoader;
...
public class ResultMap {
	 ...
	 */
	public ResultMap(SoapMapExecutorDelegate delegate) {
	...
	public void setDiscriminator(Discriminator discriminator) {
		if (this.discriminator != null) {
			throw new SoapMapException("A discriminator may only be set once per result map.");
		}
	...
	public ResultMap resolveSubMap(StatementScope statementScope, ResultSet rs) throws SQLException {
		...
		if (discriminator != null) {
			...
			if (value == null) {
				value = doNullMapping(null, mapping);
			}
	...
	public void setResultMappingList(List resultMappingList) {
		if (allowRemapping) {
			...
			this.resultMappings =
				(ResultMapping[]) resultMappingList.toArray(new ResultMapping[resultMappingList.size()]);
		}
	...
	public Object setResultObjectValues(StatementScope statementScope, Object resultObject, Object[] values) {
		...
		if (uniqueKeys != null && uniqueKeys.containsKey(ukey)) {
			// Unique key is already known, so get the existing result object and process
			// additional results.
			...
			// Unique key is NOT known, so create a new result object and then process
			// additional results.
	 ...
	 * <ul>
	 * <li>We no longer require the nested property to be a collection. This will allow reuses
	 * of resultMaps on 1:1 relationships</li>
	 * <li>If the nested property is not a collection, then it will be created/replaced by the
	 * values generated from the current row.</li>
	 * </ul>
	 * 
	 * @param mapping
	 * @param statementScope
		...
		try {
			...
			if (obj == null) {
				...
				try {
					...
					throw new SoapMapException("Error instantiating collection property for mapping '"
			...
			throw new SoapMapException("Error getting nested result map values for '" + mapping.getPropertyName()
				+ "'.  Cause: " + e, e);
		}
	}
	...
	protected Object getNestedSelectMappingValue(StatementScope statementScope, ResultSet rs, ResultMapping mapping,
		...
		try {
			...
			if (parameterType == null) {
				parameterObject = prepareBeanParameterObject(statementScope, rs, mapping, null);
			} else {
			...
			if (parameterObject != null) {
				...
				Soap sql = mappedStatement.getSql();
				ResultMap resultMap = sql.getResultMap(statementScope, parameterObject);
				Class resClass = resultMap.getResultClass();

				if (resClass != null && !DomTypeMarker.class.isAssignableFrom(targetType)) {
					if (DomCollectionTypeMarker.class.isAssignableFrom(resClass)) {
						targetType = DomCollectionTypeMarker.class;
					} else if (DomTypeMarker.class.isAssignableFrom(resClass)) {
			...
			throw new SQLException("Error setting nested bean property.  Cause: " + e, e);
	...
	protected Object getPrimitiveResultMappingValue(ResultSet rs, ResultMapping mapping) throws SQLException {
		...
		if (typeHandler != null) {
			...
			throw new SoapMapException("No type handler could be found to map the property '"
	...
	protected Object doNullMapping(Object value, ResultMapping mapping) throws SoapMapException {
		if (value == null) {
			TypeHandler typeHandler = mapping.getTypeHandler();
			if (typeHandler != null) {
				String nullValue = mapping.getNullValue();
